...
void init_spi()
{
  ros_interface = new ROS_Interface();
  ...
  std::cout << "Skip spi open" << std::endl;
  // spi_open();
}
...
void spi_driver_run()
{
  ...
  //here will be my ROS interface send - recieve

  ros_interface->sendCommand(&spi_torque);
  ros::spinOnce();
  // ROS_INFO("spi spin");
...
ROS_Interface::ROS_Interface()
{
  ROS_INFO("ROS interface constructor");

  _sub_low_state = _nh.subscribe("/low_state", 1, &ROS_Interface::_lowStateCallback, this, ros::TransportHints().tcpNoDelay(true));
  _pub_low_cmd = _nh.advertise<unitree_legged_msgs::LowCmd>("/low_cmd", 1);
}

void ROS_Interface::_lowStateCallback(unitree_legged_msgs::LowState msg)
{
  // ROS_INFO("lsc");

  for (uint8_t leg_num = 0; leg_num < 4; leg_num++)
  {
    spi_data_drv.q_abad[leg_num] = msg.motorState[leg_num * 3 + 0].q;
    spi_data_drv.q_hip[leg_num] = -msg.motorState[leg_num * 3 + 1].q;
    spi_data_drv.q_knee[leg_num] = -msg.motorState[leg_num * 3 + 2].q;

    spi_data_drv.qd_abad[leg_num] = msg.motorState[leg_num * 3 + 0].dq;
    spi_data_drv.qd_hip[leg_num] = -msg.motorState[leg_num * 3 + 1].dq;
    spi_data_drv.qd_knee[leg_num] = -msg.motorState[leg_num * 3 + 2].dq;
  }
}

void ROS_Interface::sendCommand(spi_torque_t* torque_out)
{
  static unitree_legged_msgs::LowCmd _low_cmd;

  for (uint8_t leg_num = 0; leg_num < 4; leg_num++)
  {
    _low_cmd.motorCmd[leg_num * 3 + 0].tau = torque_out->tau_abad[leg_num];
    _low_cmd.motorCmd[leg_num * 3 + 1].tau = -torque_out->tau_hip[leg_num];
    _low_cmd.motorCmd[leg_num * 3 + 2].tau = -torque_out->tau_knee[leg_num];
  }

  // ROS_INFO("low cmd pub");
