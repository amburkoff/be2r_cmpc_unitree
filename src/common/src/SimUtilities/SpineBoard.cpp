...
void SpineBoard::run()
{
  ...
  {
    torque_out[0] = kp_softstop * (q_limit_p[0] - data->q_abad[board_num]) - kd_softstop * (data->qd_abad[board_num]) + cmd->tau_abad_ff[board_num];
  }
  else if (data->q_abad[board_num] < q_limit_n[0])
  {
    torque_out[0] = kp_softstop * (q_limit_n[0] - data->q_abad[board_num]) - kd_softstop * (data->qd_abad[board_num]) + cmd->tau_abad_ff[board_num];
  }
  else
  {
  ...
  {
    torque_out[1] = kp_softstop * (q_limit_p[1] - data->q_hip[board_num]) - kd_softstop * (data->qd_hip[board_num]) + cmd->tau_hip_ff[board_num];
  }
  else if (data->q_hip[board_num] < q_limit_n[1])
  {
    torque_out[1] = kp_softstop * (q_limit_n[1] - data->q_hip[board_num]) - kd_softstop * (data->qd_hip[board_num]) + cmd->tau_hip_ff[board_num];
  }
  else
  {
