...
{
  grid_map::GridMapRosConverter::fromMessage(*msg, _grid_map_filter);
  if (!_grid_map_filter.isDefaultStartIndex())
    _grid_map_filter.convertToDefaultStartIndex();
}
...
void FSM_State_Testing_Cv<T>::LocomotionControlStep()
{
  ...
  CMPC->run(*this->_data, _grid_map_filter, _grid_map_raw);
