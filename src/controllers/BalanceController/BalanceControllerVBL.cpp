...
BalanceControllerVBL::BalanceControllerVBL() : QProblemObj_qpOASES(vblNUM_VARIABLES_QP, vblNUM_CONSTRAINTS_QP)
{
  ...
  Q1_LQR.resize(12, 12);
  Q1_LQR.setZero();
  R1_QP.resize(3 * vblNUM_CONTACT_POINTS, 3 * vblNUM_CONTACT_POINTS);
  R1_QP.setZero();
  R2_QP.resize(3 * vblNUM_CONTACT_POINTS, 3 * vblNUM_CONTACT_POINTS);
  R2_QP.setZero();
  s_LQR.resize(12, 1);
...
void BalanceControllerVBL::update_B_LQR()
{
  ...
  B_LQR.setZero();
  R_LQR.resize(3 * contact_legs, 3 * contact_legs);
  R_LQR.setZero();
...
void BalanceControllerVBL::update_P_LQR()
{
  ...
  Eigen::MatrixXcd U;
  U.setZero(2 * vblNUM_VARIABLES_QP, vblNUM_VARIABLES_QP);
  Eigen::MatrixXcd U11;
  U11.setZero(vblNUM_VARIABLES_QP, vblNUM_VARIABLES_QP);
  Eigen::MatrixXcd U21;
  U21.setZero(vblNUM_VARIABLES_QP, vblNUM_VARIABLES_QP);
  Eigen::MatrixXcd P_complex;
  P_complex.setZero(vblNUM_VARIABLES_QP, vblNUM_VARIABLES_QP);
...
void BalanceControllerVBL::calc_A_qpOASES()
{
  ...
  for (int i = 0; i < vblNUM_CONTACT_POINTS; i++)
  {
    C_control.block<1, 3>(5 * i + 0, 3 * i) << -mu_friction * .7071 * direction_normal_flatGround.transpose() + t1x.transpose();
    C_control.block<1, 3>(5 * i + 1, 3 * i) << -mu_friction * .7071 * direction_normal_flatGround.transpose() + t2y.transpose();
    C_control.block<1, 3>(5 * i + 2, 3 * i) << mu_friction * .7071 * direction_normal_flatGround.transpose() + t2y.transpose();
    C_control.block<1, 3>(5 * i + 3, 3 * i) << mu_friction * .7071 * direction_normal_flatGround.transpose() + t1x.transpose();
