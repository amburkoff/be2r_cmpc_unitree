...
mfp* vision_get_q_soln() { return vq_soln; }

s8 v_near_zero(fpt a) { return (a < 0.01 && a > -.01); }

s8 v_near_one(fpt a) { return v_near_zero(a - 1); }
                       ...
                       float x_drag)
{
  A.setZero();
  A(3, 9) = 1.f;
  A(11, 9) = x_drag; // cmpc A(11,9) old A(9,9)
  A(4, 10) = 1.f;
  A(5, 11) = 1.f;
...
void vision_quat_to_rpy(Quaternionf q, Matrix<fpt, 3, 1>& rpy)
{
  ...
  rpy(0) = atan2(2.f * (q.x() * q.y() + q.w() * q.z()),
                 vision_sq(q.w()) + vision_sq(q.x()) - vision_sq(q.y()) - vision_sq(q.z()));
  rpy(1) = asin(as);
...
void vision_solve_mpc(vision_mpc_update_data_t* update, vision_mpc_problem_setup* setup)
{
  ...
  for (int i = 0; i < num_constraints; i++)
  {
    if (!(v_near_zero(vlb_qpoases[i]) && v_near_zero(vub_qpoases[i])))
      continue;
