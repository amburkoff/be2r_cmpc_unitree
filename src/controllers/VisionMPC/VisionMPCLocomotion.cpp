...
#include "lcm-cpp.hpp"
#include "lcm_msgs/footsteps.hpp"

///////////////
///   GAIT  ///
///////////////
VisionGait::VisionGait(int nMPC_segments,
                       Vec4<int> offsets,
                       Vec4<int> durations,
...
Vec4<float> VisionGait::getContactState()
{
  ...
  {
    if (progress[i] < 0)
      progress[i] += 1.; // phase<_offsetsFloat
...
VisionMPCLocomotion::VisionMPCLocomotion(float _dt,
  ...
  , trotting(horizonLength,
             Vec4<int>(0, horizonLength / 2, horizonLength / 2, 0),
             Vec4<int>(horizonLength / 2,
                       horizonLength / 2,
                       horizonLength / 2,
                       horizonLength / 2),
             "Trotting")
  ,
  //          trotting(horizonLength, Vec4<int>(0,9,9,0),
  //          Vec4<int>(9,9,9,9),"Trotting"),
  standing(
    horizonLength,
    Vec4<int>(0, 0, 0, 0),
...
for (int t=0;t<20;t++)
std::cout<<z_list[t][0]<<z_list[t][1]<<z_list[t][2]<<z_list[t][3]<<std::endl;*/
  // 3stairs-10-5cm
...
void VisionMPCLocomotion::_UpdateFoothold(Vec3<float>& foot_pos,
...
void VisionMPCLocomotion::_IdxMapCheckingOld(int x_idx,
                                             int y_idx,
                                             int& x_idx_selected,
                                             int& y_idx_selected,
                                             const DMat<int>& idx_map)
{
  if (idx_map(x_idx, y_idx) == 0)
  { // (0,0)
    x_idx_selected = x_idx;
    y_idx_selected = y_idx;
  }
  else if (idx_map(x_idx + 1, y_idx) == 0)
  { // (1, 0)
    x_idx_selected = x_idx + 1;
    y_idx_selected = y_idx;
  }
  else if (idx_map(x_idx + 1, y_idx + 1) == 0)
  { // (1, 1)
    x_idx_selected = x_idx + 1;
    y_idx_selected = y_idx + 1;
  }
  else if (idx_map(x_idx, y_idx + 1) == 0)
  { // (0, 1)
    x_idx_selected = x_idx;
    y_idx_selected = y_idx + 1;
  }
  ...
  else if (idx_map(x_idx + 2, y_idx - 1) == 0)
  { // (2, -1)
    x_idx_selected = x_idx + 2;
    y_idx_selected = y_idx - 1;
  }
  else if (idx_map(x_idx + 2, y_idx) == 0)
  { // (2, 0)
    x_idx_selected = x_idx + 2;
    y_idx_selected = y_idx;
  }
  ...
  else if (idx_map(x_idx + 2, y_idx + 2) == 0)
  { // (2, 2)
    x_idx_selected = x_idx + 2;
...
void VisionMPCLocomotion::_UpdateFootholdNew(Vec3<float>& foot_des,
                                             Vec3<float>& foot_ini,
                                             const Vec3<float>& body_pos,
                                             const DMat<float>& height_map,
                                             const DMat<float>& idx_map)
{
  Vec3<float> pf = foot_des - body_pos;
  pf[0] = 0.; // useless
  ...
  foot_ini_tmp[0] =
    foot_ini_tmp[0] - camera_offset; // foot pos relative to camera
  ...
  {
    _IdxMapChecking(x_idx, y_idx, x_idx_selected, y_idx_selected, height_map,
                    idx_map, foot_ini_tmp);
 ...
 * */
bool VisionMPCLocomotion::_SurroundingChecking(int x_idx,
                                               int y_idx,
                                               const DMat<float>& idx_map)
{ // 1123
  ...
  if (s01 <= 1.5 && s0_1 <= 1.5 && s10 <= 1.5 && s11 <= 1.5 && s1_1 <= 1.5 &&
      s_10 <= 1.5 && s_11 <= 1.5 && s_1_1 <= 1.5)
  {
    surrounding_OK = true;
  }
  else
 ...
 * */
bool VisionMPCLocomotion::_FeasibilityChecking(int x_idx,
                                               int y_idx,
                                               const DMat<float>& height_map,
                                               Vec3<float>& foot_ini)
{ // 1123
  ...
  if (height_map(x_idx, y_idx) > height_map(x_idx_last, y_idx_last) &&
      height_map(x_idx, y_idx) - height_map(x_idx_last, y_idx_last) >=
        StepHeight * .7)
  {
    feasibility_OK = false;
    std::cout << " current: " << height_map(x_idx_last, y_idx_last)
              << "next: " << height_map(x_idx, y_idx) << std::endl;
  }
  else if (height_map(x_idx, y_idx) < height_map(x_idx_last, y_idx_last) &&
           height_map(x_idx_last, y_idx_last) - height_map(x_idx, y_idx) >=
             StepHeight * .5)
  {
    feasibility_OK = false;
    std::cout << " current: " << height_map(x_idx_last, y_idx_last)
 ...
 * */
void VisionMPCLocomotion::_IdxMapChecking(int x_idx,
                                          int y_idx,
                                          int& x_idx_selected,
                                          int& y_idx_selected,
                                          const DMat<float>& height_map,
...
{ // 1119
  checkStep_OK = true;
  if (idx_map(x_idx, y_idx) <= 1.5 &&
      _FeasibilityChecking(x_idx, y_idx, height_map, foot_ini) &&
      _SurroundingChecking(x_idx, y_idx, idx_map))
  { // (0,0)
    x_idx_selected = x_idx;
    y_idx_selected = y_idx;
  }
  else if (idx_map(x_idx + 1, y_idx) <= 1.5 &&
           _FeasibilityChecking(x_idx + 1, y_idx, height_map, foot_ini) &&
           _SurroundingChecking(x_idx + 1, y_idx, idx_map))
  { // (1, 0)
    x_idx_selected = x_idx + 1;
    y_idx_selected = y_idx;
  }
  ...
  else if (idx_map(x_idx, y_idx + 1) <= 1.5 &&
           _FeasibilityChecking(x_idx, y_idx + 1, height_map, foot_ini) &&
           _SurroundingChecking(x_idx, y_idx + 1, idx_map))
  { // (0, 1)
    x_idx_selected = x_idx;
    y_idx_selected = y_idx + 1;
  }
  ...
  else if (idx_map(x_idx + 1, y_idx - 1) <= 1.5 &&
           _FeasibilityChecking(x_idx + 1, y_idx - 1, height_map, foot_ini) &&
           _SurroundingChecking(x_idx + 1, y_idx - 1, idx_map))
  { // (1, -1)
    x_idx_selected = x_idx + 1;
    y_idx_selected = y_idx - 1;
  }
  ...
  else if (idx_map(x_idx - 2, y_idx) <= 1.5 &&
           _FeasibilityChecking(x_idx - 2, y_idx, height_map, foot_ini) &&
           _SurroundingChecking(x_idx - 2, y_idx, idx_map))
  { // (-2, 0)
    x_idx_selected = x_idx - 2;
    y_idx_selected = y_idx;
  }
  else if (idx_map(x_idx - 2, y_idx - 1) <= 1.5 &&
           _FeasibilityChecking(x_idx - 2, y_idx - 1, height_map, foot_ini) &&
           _SurroundingChecking(x_idx - 2, y_idx - 1, idx_map))
  { // (-2, -1)
    x_idx_selected = x_idx - 2;
    y_idx_selected = y_idx - 1;
  }
  ...
  { // (2, 1)
    ...
    //    }else if(idx_map(x_idx-3, y_idx) <= 1.5 &&
    //    _SurroundingChecking(x_idx-3,y_idx,idx_map)){ // (-3, 0)
    //        x_idx_selected = x_idx-3;
    //        y_idx_selected = y_idx;
    //    }else if(idx_map(x_idx-3, y_idx-1) <= 1.5 &&
    //    _SurroundingChecking(x_idx,y_idx,idx_map)){ // (-3, -1)
    //        x_idx_selected = x_idx-3;
    //        y_idx_selected = y_idx-1;
    //    }else if(idx_map(x_idx-3, y_idx+1) <= 1.5 &&
    //    _SurroundingChecking(x_idx,y_idx,idx_map)){ // (-3, 1)
  ...
  { // if cannot find proper footstep, use the step having the best
    // traversablility score
    ...
    printf("Final select (%d, %d):score %f\n\n", x_idx_selected, y_idx_selected,
           idx_map(x_idx_selected, y_idx_selected));
    checkStep_OK = false;
  }
}
...
void VisionMPCLocomotion::run(ControlFSMData<float>& data,
                              const Vec3<float>& vel_cmd,
                              const DMat<float>& height_map,
                              const DMat<float>& idx_map)
{ // 1119
  //    const Vec3<float> & vel_cmd, const DMat<float> & height_map, const
  //    DMat<int> & idx_map) {
  (void)idx_map;
  ROS_INFO_STREAM("Get heightmap with shape ["
                  << height_map.rows() << height_map.cols() << std::endl);
  ...
  {
    //        if (std::abs(pfoot_ini[0][2]-pfoot_ini[3][2])<=0.01)
    //        BodyHeight=0.29+pfoot_ini[0][2];//1124
  ...
  for (int i = 0; i < 4; i++)
  {
    pFoot[i] = seResult.position + // pos in world frame
               seResult.rBody.transpose() *
                 (data._quadruped->getHipLocation(i) + // pos in body frame
  ...
  if (gait != &standing)
  {
    ...
    //        if
    //        (x_ini_idx<100&&x_ini_idx>0&&BodyHeight-height_map(x_ini_idx,y_ini_idx)<0.25)
    //        BodyHeight=height_map(x_ini_idx,y_ini_idx)+0.25;
    ////        else if(BodyHeight-height_map(x_ini_idx,y_ini_idx)<0.25)
    /// BodyHeight=0.29;
    //        else if (std::abs(pfoot_ini[0][2]-pfoot_ini[3][2])<=0.02)
    //        BodyHeight=0.29+height_map(x_ini_idx,y_ini_idx); else
    //        BodyHeight=0.29; if
    //        (std::abs(seResult.position[2]-BodyHeight)>0.02)
    //            seResult.position[2]=BodyHeight;
    //        std::cout<<seResult.position[2]<<" "<<x_ini_idx<<"
    //        "<<y_ini_idx<<std::endl;
  }
  // gaitstate
  ...
  if (firstRun)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      ...
      _UpdateFootholdNew(pfoot_des[i], pfoot_ini[i], seResult.position,
                         height_map, idx_map);
    }
  ...
  for (int foot = 0; foot < 4; foot++)
  {
    ...
    if (swingState > 0) // foot is in swing
    {
      ...
      //        std::cout<<foot<<" ini "<<pfoot_ini[foot][0]<<" des
      //        "<<pfoot_des[foot][0]<<" "<<swingState<<std::endl;
      ...
      Vec3<float> pDesLeg =
        seResult.rBody * (pDesFootWorld[foot] - seResult.position) -
      ...
      if (!data.userParameters->use_wbc)
      {
        ...
        data._legController->commands[foot].tauFeedForward[2] =
          ...
          data._legController->datas[foot].q(2);
      }
    }
    else // foot is in stance
    {
      if (firstSwing[foot] == false)
      {
        ...
        pfoot_ini[foot] = pFoot[foot];
        //  update body height
        if (pfoot_des[foot][2] > pzFootMin &&
            pfoot_des[foot][2] - pzFootMin > 0.01)
        {
          pzFootMin = pfoot_des[foot][2];
          BodyHeight = pzFootMin + 0.27;
          ROS_INFO_STREAM("Body height changed to " << BodyHeight);
        }
        //  update body pitch
        double tan_p = 0.0;
        if (foot == 1)
        {
          tan_p =
            (pFoot[foot][2] - pFoot[2][2]) / (pFoot[foot][0] - pFoot[2][0]);
        }
        else if (foot == 0)
        {
          tan_p =
            (pFoot[foot][2] - pFoot[3][2]) / (pFoot[foot][0] - pFoot[3][0]);
        }
        if (std::abs(tan_p) > 0.10)
          rpy_comp[1] += -atan(tan_p);
        ...
        if (stepcount[2] >= sumStep)
        {
          ...
          vx = 0.0;
          //                StepHeight=0.;
          //                _UpdateFootholdNew(pfoot_des[foot],
          //                seResult.position, height_map, idx_map);
        }
        else
        {
          //           pfoot_des[foot] = pDesFootWorld[foot] + delta;
          pfoot_log[foot] =
            pfoot_log[foot] + delta; // next step's nominal xy location
          pfoot_des[foot] = pfoot_log[foot];
          _UpdateFootholdNew(pfoot_des[foot], pfoot_ini[foot],
                             seResult.position, height_map, idx_map);
      ...
      {
        stepcount[foot]++;
        //        std::cout << foot << " ini-x " << pfoot_ini[foot][0] << "
        //        nom-x "
        //                  << pfoot_log[foot][0] << " des-x " <<
        //                  pfoot_des[foot][0];
        //        std::cout << " ini-z " << pfoot_ini[foot][2] << " des-z "
        //                  << pfoot_des[foot][2] << " step" << stepcount[foot]
        //                  << " pitch " << rpy_comp[1] << std::endl;
        if (!checkStep_OK)
          std::cout << "no proper step found!" << std::endl;
      }
      //      std::cout<<foot<<" "<<pfoot_ini[foot][1]<<"
      //      "<<pfoot_des[foot][1]<<std::endl;
      Vec3<float> vDesFootWorld = footSwingTrajectories[foot].getVelocity();
      Vec3<float> pDesLeg =
        seResult.rBody * (pDesFootWorld[foot] - seResult.position) -
        data._quadruped->getHipLocation(foot);
...
void VisionMPCLocomotion::updateMPCIfNeeded(int* mpcTable,
  ...
  if ((iterationCounter % iterationsBetweenMPC) == 0)
  {
    ...
    else
    {
      ...
      for (int i = 0; i < horizonLength; i++)
      {
        ...
        {
          trajAll[12 * i + 3] =
            trajAll[12 * (i - 1) + 3] + dtMPC * vx; // v_des_world[0];
          trajAll[12 * i + 4] =
            trajAll[12 * (i - 1) + 4] + dtMPC * v_des_world[1];

          trajAll[12 * i + 2] =
            trajAll[12 * (i - 1) + 2] + dtMPC * v_rpy_des[2];
        }
      }
    }
...
void VisionMPCLocomotion::solveDenseMPC(int* mpcTable,
                                        ControlFSMData<float>& data)
{
  auto seResult = data._stateEstimator->getResult();
  ...
  dtMPC = dt * iterationsBetweenMPC;
  vision_setup_problem(dtMPC, horizonLength, 0.4, 120);
  vision_update_problem_data_floats(p, v, q, w, r, yaw, weights, trajAll, alpha,
                                    mpcTable);
