...
void vision_update_problem_data(double* p, double* v, double* q, double* w, double* r, double yaw,
...
void vision_update_problem_data_floats(float* p, float* v, float* q, float* w, float* r, float yaw,
                                       float* weights, float* state_trajectory, float alpha,
                                       int* gait)
{
  v_update.alpha = alpha;
  v_update.yaw = yaw;
  ...
  memcpy((void*)v_update.traj, (void*)state_trajectory,
         sizeof(float) * 12 * v_problem_config.horizon);
  vision_solve_mpc(&v_update, &v_problem_config);
  vision_has_solved = 1;
}
...
void vision_update_solver_settings(int max_iter, double rho, double sigma, double solver_alpha,
                                   double terminate, double use_jcqp)
{
  v_update.max_iterations = max_iter;
  v_update.rho = rho;
  v_update.sigma = sigma;
  v_update.solver_alpha = solver_alpha;
  v_update.terminate = terminate;
  if (use_jcqp > 1.5)
    v_update.use_jcqp = 2;
