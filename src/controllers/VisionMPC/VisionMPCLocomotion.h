...
public:
  ...
  //      const Vec3<T> & vel_cmd, const DMat<T> & height_map, const DMat<int> &
  //      idx_map);
  template<typename T>
  void run(ControlFSMData<T>& data,
           const Vec3<T>& vel_cmd,
           const DMat<T>& height_map,
           const DMat<float>& idx_map); // float
...
private:
  void _UpdateFoothold(Vec3<float>& foot,
                       const Vec3<float>& body_pos,
                       const DMat<float>& height_map,
                       const DMat<int>& idx_map);
  void _IdxMapCheckingOld(int x_idx,
                          int y_idx,
                          int& x_idx_selected,
                          int& y_idx_selected,
                          const DMat<int>& idx_map);
  // 1119
  void _UpdateFootholdNew(Vec3<float>& foot_des,
                          Vec3<float>& foot_ini,
                          const Vec3<float>& body_pos,
                          const DMat<float>& height_map,
                          const DMat<float>& idx_map);
  void _IdxMapChecking(int x_idx,
                       int y_idx,
                       int& x_idx_selected,
                       int& y_idx_selected,
                       const DMat<float>& height_map,
  ...
  bool _FeasibilityChecking(int x_idx,
                            int y_idx,
                            const DMat<float>& height_map,
                            Vec3<float>& foot_ini);
