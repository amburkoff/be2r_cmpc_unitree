...
void Metric::computeCenterLegVelAndPos(Quadruped<float> &quad, Vec3<float> &q, Vec3<float> &dq, Mat3<float> *J, Mat3<float> *leg_p, Mat3<float> *leg_v, Mat3<float> *leg_w, int leg)
{
  ...
  if ((J) && (leg_p) && (leg_v))
  {
    for (int i = 0; i < 3; i++)
    {
      if (i ==1)
      {
        ...
        leg_p->operator()(0,i) = l3 * s23 + l2 * s2;
        leg_p->operator()(1,i) = (l1 + l4) * sideSign * c1 + l3 * (s1 * c23) + l2 * c2 * s1;
        leg_p->operator()(2,i) = (l1 + l4) * sideSign * s1 - l3 * (c1 * c23) - l2 * c1 * c2;
      }
      leg_v->block(0,i,3,1) = *(J)*dq;
    }
  }
  else if (J)
  {
