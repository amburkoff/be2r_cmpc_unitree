...
void CMPCLocomotion_Cv::recompute_timing(int iterations_per_mpc)
{
...
void CMPCLocomotion_Cv::_SetupCommand(ControlFSMData<float>& data)
{
  ...
  _pitch_cmd = 0.4 * data._desiredStateCommand->rightAnalogStick[1];
                                  ...
                                  const grid_map::GridMap& height_map_raw)
{
  ...
  else
  {
    ...
    _pitch_des = _pitch_cmd + data._stateEstimator->getResult().rpy[1] + data._stateEstimator->getResult().est_pitch_plane -
                 0.07 * _x_vel_des; // _pitch_des = data._stateEstimator->getResult().est_pitch_plane;
  }
  ...
  for (int foot = 0; foot < 4; foot++)
  {
    ...
    else // foot is in stance
    {
      ...
      data.debug->last_p_local_stance[foot] =
        ros::toMsg(data._legController->datas[foot].p + data._quadruped->getHipLocation(foot));
...
void CMPCLocomotion_Cv::updateMPCIfNeeded(int* mpcTable, ControlFSMData<float>& data, bool omniMode)
{
...
void CMPCLocomotion_Cv::solveDenseMPC(int* mpcTable, ControlFSMData<float>& data)
{
  auto seResult = data._stateEstimator->getResult();
...
// void CMPCLocomotion_Cv::solveSparseMPC(int* mpcTable, ControlFSMData<float>& data)
// {
...
void CMPCLocomotion_Cv::initSparseMPC()
{
