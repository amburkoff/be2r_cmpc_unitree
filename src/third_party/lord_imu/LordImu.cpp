...
void LordImu::init(u32 port, u32 baud_rate)
{
  ...
  // _sub_imu = _nh.subscribe("/imu", 1, &LordImu::_imuCallback, this);
  _sub_low_state = _nh.subscribe("/low_state", 1, &LordImu::_lowStateCallback, this, ros::TransportHints().tcpNoDelay(true));
}
...
// void LordImu::_imuCallback(unitree_legged_msgs::IMU msg)
// {
//   // ROS_INFO("imu callback");

//   gyro[0] = msg.gyroscope[0];
//   gyro[1] = msg.gyroscope[1];
//   gyro[2] = msg.gyroscope[2];

//   acc[0] = msg.accelerometer[0];
//   acc[1] = msg.accelerometer[1];
//   acc[2] = msg.accelerometer[2];

//   quat[0] = msg.quaternion[0];
//   quat[1] = msg.quaternion[1];
//   quat[2] = msg.quaternion[2];
//   quat[3] = msg.quaternion[3];
// }

void LordImu::_lowStateCallback(unitree_legged_msgs::LowState msg)
{
  // ROS_INFO("imu callback");

  gyro[0] = msg.imu.gyroscope[0];
  gyro[1] = msg.imu.gyroscope[1];
  gyro[2] = msg.imu.gyroscope[2];

  acc[0] = msg.imu.accelerometer[0];
  acc[1] = msg.imu.accelerometer[1];
  acc[2] = msg.imu.accelerometer[2];

  quat[0] = msg.imu.quaternion[0]; //w
  quat[1] = msg.imu.quaternion[1]; //x
  quat[2] = msg.imu.quaternion[2]; //y
  quat[3] = msg.imu.quaternion[3]; //z

  // cout << "imu callback qu0: " << quat[0] << " qu1: " << quat[1] << " qu2: " << quat[2] << " qu3: " << quat[3] << endl;
}
